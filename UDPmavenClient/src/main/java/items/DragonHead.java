...
public class DragonHead extends Item {
    ...
    public static DragonHead consoleRead(){
        Scanner input;
        ...
        System.out.println("Введите данные для DragonHead:");

        boolean flag = true;
        while (flag)
            try {
                size = Long.valueOf(CorrectedInput.correctedConsoleInput("size", input));
                flag = false;
        ...
        while (flag)
            try {
                eyesCount = Float.valueOf(CorrectedInput.floatConsoleInput("eyesCount", input));
                flag = false;
            } catch (NumberFormatException e){
                try{
                    throw new WrongNumberException();
