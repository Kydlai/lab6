...
public class Dragon extends Item {
     ...
     */
    public Dragon(String name, Coordinates coordinates, int age, String description, DragonType type, DragonCharacter character, DragonHead head, Long id) {
    ...
    public static Dragon fileRead(){
        ...
        while (dragonType == null) {
            ...
            dragonType = Dragon.typeReader(inputString[Server.fileReaderCounter - 1]);
        }

        while (dragonCharacter == null) {
            Server.fileReaderCounter++;
            dragonCharacter = Dragon.characterReader(inputString[Server.fileReaderCounter - 1]);
        }

        Coordinates coordinates = Coordinates.fileRead();
        DragonHead head = DragonHead.fileRead();

        dragon = new Dragon(name, coordinates, age, description, dragonType, dragonCharacter, head, Server.idCounter++);
